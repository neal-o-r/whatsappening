...
def make_plots(df):
	...
	plt.tight_layout()	
	#ax4.set_xticklabels(times.strftime('%b - %Y')[::1200], rotation=70)

	df.groupby('Hour').count().plot(ax=ax1, legend = None)
...
def plot_sentiment(df):
	...
	ax.set_xticklabels(times.strftime('%b - %y'), rotation='vertical', size = 'small')

	df.groupby(times.date).mean().plot(y='sentiment', ylim=[-1.5,1.5], legend=None, xlim=[times.date[0],times.date[-1]], ax=ax)	
...
if __name__ == '__main__':
	...
	processed_df = process(content[::5])
